...
class InteractionController(base: Pos, dir: Vec, instance: Instance, w: Int = 1, h: Int = 1, r: Double = 0.1, val inv: Boolean, val cb: (Int, Int, Pos, Player) -> Unit) {
    ...
    companion object {
        ...
        val lastInteraction = Tag.Long("last_interaction")
        val debounceMs = 100

        fun eventNode() = EventNode.all("map")
            .listen { event: PlayerEntityInteractEvent ->
                if ((event.player.getTag(lastInteraction) ?: 0) >= System.currentTimeMillis() - debounceMs) {
                    return@listen
                }
                event.player.setTag(lastInteraction, System.currentTimeMillis())
                ...
                controller.cb(x * 2 + dx, y * 2 + dy, entity.position.add(iHat.mul(0.5 * dx)).add(jHat.mul(0.5 * dy)), event.player)
            }
    }
}